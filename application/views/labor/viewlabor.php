...
<section class="content">
    <div class="row">
        <div class="col-xs-12">
          ...
          <h3 class="box-title">Statistic & Graph - Laboratory</h3>
                  ...
                  <div class="box-body">
                    ...
                    <table id="exampl" class="table table-striped table-bordered table-hover table-full-width dataTable"
                      ...
                      <thead>
                        <tr class="bg-success">
                          <th width="12%">No</th>
                          <th width="77%" class="table_title">Habitat Type</th>

                          <th width="29%">Bulk density</th>
                          <th width="29%">Porosity</th>
                          <th width="29%">Agregate stability</th>
                          <th width="29%">Field capacity</th>
                          <th width="29%">Wilting point</th>
...
<script>
  ...
  function getFilter() {
    $.ajax({
      type: "POSt",
      url: url + "Chartlabor/filterSelect",
      data: "data",
      dataType: "JSON",
  ...
  function showTableHabitat(response) {
    ...
    response.data_chart.detail.forEach(element => {
      html += ` <tr>
                      <td>` + number++ + `</td>
                      <td>` + element.label + `</td>
                      <td>` + element.bulk_density + `</td>
                      <td>` + element.porosity + `</td>
                      <td>` + element.agregate + `</td>
                      <td>` + element.field_capacity + `</td>
                      <td>` + element.wilting_point + `</td>
                    </tr>`;
    });
...

      {
        ...
        datasets: [

          {
            ...
            data                  : data.data_chart.data.a_bulk_density
          },
          
          {
            label                 : 'Porosity',
            backgroundColor       : '#dee605',         
            data                  : data.data_chart.data.a_porosity
          },
          {
            label                 : 'Agregate',
            backgroundColor       : '#1327bf',         
            data                  : data.data_chart.data.a_agregate
          },
          {
            label                 : 'Field Capacity',
            backgroundColor       : '#a2b0af',         
            data                  : data.data_chart.data.a_field_capacity
          },
          {
            label                 : 'Wilting Point',
            backgroundColor       : '#057c82',         
            data                  : data.data_chart.data.a_wilting_point
          },
